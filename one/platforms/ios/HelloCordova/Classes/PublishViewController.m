...
#define UpLoadNotification @"IMAGEUPLOADFINISHNOTIFICATION"

static int succeededImage = 0;
static int errorImage = 0;
...
-(void)pushSec{
    if (self.freekTextView.text.length<1) {
        [SVProgressHUD showErrorWithStatus:@"要输入发表的心情哦"];
        return;
    }
    if(_imageArray.count<1){
        [SVProgressHUD showErrorWithStatus:@"要加入图片说明哦"];
        return;
    }
    ...
    succeededImage = 0;
    errorImage = 0;
    allImage = _imageArray.count;
    imageUrlArray = nil;
    imageUrlArray = [NSMutableArray array];
    [SVProgressHUD showWithStatus:@"正在发表"];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(uploadFinish) name:UpLoadNotification object:nil];
    for (UIImage *image in _imageArray) {
        ...
        [dic setObject:[NSString stringWithFormat:@"%.2f",image.size.width] forKey:@"image_width"];
        [dic setObject:[NSString stringWithFormat:@"%.2f",image.size.height] forKey:@"image_height"];
        [file uploadWithCompletionHandler:^(BOOL succeeded, NSError * _Nullable error) {
            if (succeeded) {
                succeededImage++;
                [dic setObject:file.url forKey:@"image_url"];
                [imageUrlArray addObject: dic];
                [self checkCount:allImage];
            }else{
                errorImage++;
                [dic setObject:(file.url ? file.url : @"" ) forKey:@"image_url"];
                [imageUrlArray addObject: dic];
                [self checkCount:allImage];
            }
        }];
...
-(void)checkCount:(long)sumNumber{
    if (succeededImage+errorImage == sumNumber ) {
        [[NSNotificationCenter defaultCenter] postNotificationName:UpLoadNotification object:nil userInfo:nil];
        [[NSNotificationCenter defaultCenter] removeObserver:self name:UpLoadNotification object:nil];
    }else{
        NSLog(@"NO FINISH");
    }
}
...
-(void)uploadFinish{
    ...
    [object setObject:imageUrlArray forKey:@"images"];
    [object saveInBackgroundWithBlock:^(BOOL succeeded, NSError * _Nullable error) {
        if (succeeded) {
            [SVProgressHUD showSuccessWithStatus:@"发表成功！"];
            self.freekTextView.text = nil;
