...
- (void)creatSubView {
    ...
    UILabel *label = [[UILabel alloc]initWithFrame:visulEffectView.frame];
    label.tag = 30;
    label.font = [UIFont systemFontOfSize:10];
    label.textColor = [UIColor whiteColor];
    label.textAlignment = NSTextAlignmentCenter;
    [visulEffectView.contentView addSubview:label];
}
...
//    AVFile *file = [AVFile fileWithRemoteURL:[NSURL URLWithString:_model.imgURL]];
//    [file getThumbnail:YES width:imgV.frame.size.width height:imgV.frame.size.height withBlock:^(UIImage *image, NSError *error) {
//        imgV.image = image;
//    }];
