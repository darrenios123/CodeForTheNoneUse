...
- (void)viewDidLoad {
    ...
    AVQuery *query = [AVQuery queryWithClassName:@"userTimeLine"];
    [query whereKeyExists:@"images"];
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        NSMutableArray * arrayUser = [NSMutableArray arrayWithArray:objects];
        for (NSDictionary * dicUser in arrayUser) {
            NSMutableArray * array = [dicUser objectForKey:@"images"];
            for (NSDictionary *dic in array) {
                CellModel *model = [[CellModel alloc]init];
                model.title = dicUser[@"abs"];
...
//
//    NSString *urlStr = [@"http://image.baidu.com/channel/listjson?pn=0&rn=50&tag1=摄影&tag2=全部&ie=utf8" stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
//
...
- (void)_creatSubView {
    ...
    self.wslayout.rowSpacing = 2; //行间距
    self.wslayout.lineSpacing = 2; //列间距
    ...
    self.collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-64) collectionViewLayout:self.wslayout];
