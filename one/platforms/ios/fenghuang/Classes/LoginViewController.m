...
- (void)loginBtnClick
{
    [AVUser logInWithMobilePhoneNumberInBackground:_nameTextField.text password:_pwdTextField.text block:^(AVUser *user, NSError *error) {
        if (!error) {
            [self dismissViewControllerAnimated:YES completion:^{}];
        }else{
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"登陆失败" message:error.description delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
            [alert show];
        }
    }];
}
